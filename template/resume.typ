#let resume(
  ...
  align(author-position, [
    #text(text(font-size + 16pt, weight: "extrabold")[#author-name])
    #v(-2em)
  ])
...
#let work-heading(title, company, location, start-date, end-date, body) = {
  ...
  show link: set text(blue, weight: "bold")
...
// Pretty self-explanatory.
#let project-heading(name, repository-url, start-date, end-date, body) = {
  // sanity checks
  assert.eq(type(start-date), datetime)
  assert(type(end-date) == datetime or type(end-date) == str)
  ...
  generic_1x2(
    [*#name | #emph(link(repository-url, "Repository"))*],
    [#period_worked(start-date, end-date)],
  )
